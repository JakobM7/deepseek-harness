...
describe('web e2e: Models settings page configures a dormant provider', () => {
  ...
  beforeAll(async () => {
    ...
    // The product ships the Models section in English.
  ...
  it('refuses a key no HTTP header can carry before anything is written', async () => {
    ...
    await key.blur()
    await expect.poll(() => key.getAttribute('aria-invalid'), { timeout: 10_000 }).toBe('true')
    ...
    await expect.poll(() => key.getAttribute('aria-invalid'), { timeout: 10_000 }).not.toBe('true')
  }, 60_000)
