...
describe('web e2e: settings modal and General preferences', () => {
  ...
  beforeAll(async () => {
    ...
    // The product has one shipped locale, so every browser gets the English
    // settings surface.
  ...
  it('opens the settings dialog, switches sections, and closes by every path', async () => {
    ...
    expect(await dialog.getByRole('button', { name: 'General', exact: true }).getAttribute('aria-current')).toBe('true')
    ...
    // Golden of the freshly opened dialog (General active).
    ...
    expect(await dialog.getByRole('button', { name: 'General', exact: true }).getAttribute('aria-current')).toBeNull()
  ...
  it('opens an English browser in English without any stored preference', async () => {
    // A fresh Host home has no locale preference, and the product still opens
    // with its deterministic English surface.
  ...
  it('opens a browser asking for no shipped language in English', async () => {
    // Browser language does not change the product's deterministic English
    // surface.
    const fresh = await launchWebScaffold({})
    ...
    try {
      ...
      // Asserted here too so a future change that writes the wrong tag is caught.
      ...
      // produces; this is the same English surface as the default browser.
