  ...
  - list:
    ...
    - listitem:
      - 'button "Set as default: PTC mode"':
        - text: PTC mode Built-in All Standard mode capabilities, with tools exposed through the Code Mode SDK so the model can combine multi-step operations in one TypeScript program.
        - code: code
  ...
  - list:
    - listitem:
      ...
      - 'button "Delete: broken-yaml"':
        ...
        - text: Delete
    - listitem:
      - 'button "Failed to load: 幽灵预设" [disabled]':
        - text: 幽灵预设 Failed to load Custom composition 已被手动删除。
        - alert: the composition file agent.cordis.yml is missing — the directory still occupies the id; delete it or restore the file
        - code: ghost
      - 'button "Show location: 幽灵预设"':
        - img
        - text: Show location
      - 'button "Duplicate: 幽灵预设" [disabled]':
        - img
        - text: A preset that failed to load cannot be duplicated
      - 'button "Delete: 幽灵预设"':
        - img
