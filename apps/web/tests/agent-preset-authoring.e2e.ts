...
import { EN_BROWSER_LOCALE, connectFreshWorkspaceEn, saveFailureShot } from './support.ts'
...
describe('web e2e: agent-preset authoring is a host-side copy', () => {
  ...
  function withPresetRoot(snapshot: string): string {
    ...
    return snapshot.replaceAll('\\', '/').split('\n').map((line) => {
  ...
  beforeAll(async () => {
    ...
    // The product ships the preset section in English.
  ...
  it('deletes the copy after confirmation and reclaims the roster', async () => {
    ...
    const dialog = settingsDialog()
    await dialog.getByRole('button', { name: 'Delete: 我的模式' }).click()
  ...
  it('marks damaged presets broken and clears a ghost through delete', async () => {
    ...
    expect(snapshot).toContain('Failed to load: 幽灵预设')
    ...
    expect(await dialog.getByRole('button', { name: 'Duplicate: 幽灵预设' }).isDisabled()).toBe(true)
    ...
    await dialog.getByRole('button', { name: 'Delete: 幽灵预设' }).click()
    const confirm = page.getByRole('dialog', { name: 'Delete this preset?' })
    await confirm.waitFor({ timeout: 10_000 })
    await confirm.getByRole('button', { name: 'Delete', exact: true }).click()
    await confirm.waitFor({ state: 'detached', timeout: 10_000 })
  ...
  it('starts a creator-mode session from the section', async () => {
    ...
    await connectFreshWorkspaceEn(page, scaffold.workspaceCwd)
    await page.getByRole('button', { name: 'Settings', exact: true }).click()
    const dialog = settingsDialog()
    await dialog.waitFor({ timeout: 10_000 })
    await dialog.getByRole('button', { name: 'Agent presets' }).click()
