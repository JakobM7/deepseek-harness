 ...
 * card holds. The unit — `K matches · N files` for grep, `N paths` for glob — trails
...
function summaryText(props: SearchBlockProps, shown: number, truncated: boolean, total: number): string {
  ...
  return props.kind === 'paths'
    ? `${count} paths`
    : `${count} matches · ${props.files.length} file${props.files.length === 1 ? '' : 's'}`
}
          ...
          <div className={css.body}>
            ...
            {hidden > 0 && (
              <button
                ...
                aria-label={expanded ? 'Collapse results' : `Expand remaining ${hidden} result lines`}
                onClick={onToggle}
              >
